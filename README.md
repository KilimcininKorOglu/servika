...
| `SERVIKA_SECRET_KEY`       | none, required                                                                                                    | AES-256-GCM key (at least 32 characters) for encrypting credentials/tokens at rest (FTP, GitHub PAT, backup-destination passwords). The server refuses to boot without it. |
...
| `SERVIKA_REPO`              | `KilimcininKorOglu/servika`                                          | `install.sh`, `servika-update`, `servika-restore`        | GitHub repository used to download release bundles.                                                   |
