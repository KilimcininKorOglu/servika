...
Production values live in `/etc/servika/env`. The systemd unit loads that file before starting `servika-server`. Keep the file mode at `0600` because it contains database, JWT, and Redis secrets.
...
| `SERVIKA_PUBLIC_IPV4` | first non-loopback IPv4 address | Public IPv4 override for webhook URLs, SSH display, and DNS seed records. |
| `SERVIKA_MAINTENANCE_MODE` | unset | Returns HTTP 503 for most requests when set to `1` or `true`. Health and login endpoints stay available. |
...
|----------|---------|---------|---------|
| `SERVIKA_DB_PASS` | parsed from `SERVIKA_DB_DSN` when absent | `servika-ftp-setup` | Optional panel database password override for Pure-FTPd setup. |
| `SERVIKA_SEED_PASSWORD` | none | `scripts/seed_admin.go` | Administrator password fallback when `-password` is omitted. |
| `SERVIKA_REPO` | `KilimcininKorOglu/servika` | `servika-update`, `servika-restore` | GitHub repository used to download release assets. |
| `SERVIKA_PREFIX` | `/opt/servika` | `servika-restore` | Root path for restored panel files. |
| `SERVIKA_BIN` | `$SERVIKA_PREFIX/bin/servika-server` | `servika-restore` | Target server binary path. |
| `SERVIKA_SEED` | `$SERVIKA_PREFIX/bin/servika-seed-admin` | `servika-restore` | Target seed-admin binary path. |
| `SERVIKA_FDIST` | `$SERVIKA_PREFIX/frontend-dist` | `servika-restore` | Target frontend distribution path. |
| `SERVIKA_MIGR` | `$SERVIKA_PREFIX/src/migrations` | `servika-restore` | Target migrations path. |
| `SERVIKA_SCRIPTS` | `$SERVIKA_PREFIX/src/scripts` | `servika-restore` | Target build-time scripts path. |
| `SERVIKA_OPSBIN` | `/usr/local/bin` | `servika-restore` | Target operations binary directory. |
| `SERVIKA_SVC` | `servika` | `servika-restore` | systemd service name used during repair and health checks. |
| `SERVIKA_HEALTH` | `http://127.0.0.1:8080/healthz` | `servika-restore` | Health check URL used after restoring core files. |
| `SERVIKA_DBBK` | `/usr/local/bin/servika-db-backup` | `servika-restore` | Panel database dump tool path. |
| `SERVIKA_DBDIR` | `/var/backups/servika/db` | `servika-restore` | Panel database dump directory. |
| `SERVIKA_ASSETS_OVERRIDE` | unset | `servika-restore` | Local assets directory for isolated or offline restore tests. |
