...
export default function ServerStatusPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .catch((e) => { if (!cancelled) setError(apiError(e, 'Could not load server status')) })
  ...
  return (
    <div className="w-full px-4 py-6">
      ...
      <div className="mb-5">
        <h1 className="text-2xl font-semibold text-slate-900 dark:text-slate-100">Server Status</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
          A live snapshot of the server. This screen is read-only — service management belongs to the administrator.
        </p>
      </div>
      ...
      {loading ? (
        ...
        <>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
            <Card title="CPU" value={pct(usage?.cpu?.percent)} sub={loadText} />
            <Card title="Memory" value={pct(usage?.memory?.percent)}
                  sub={usage?.memory?.total_kb ? `${Math.round((usage.memory.used_kb ?? 0) / 1024)} / ${Math.round(usage.memory.total_kb / 1024)} MB` : undefined} />
            <Card title="Disk" value={pct(usage?.disk?.percent)}
                  sub={usage?.disk?.total_byte ? `${gb(usage.disk.used_byte)} / ${gb(usage.disk.total_byte)} GB` : undefined} />
            <Card title="Panel Version" value={usage?.system?.panel_version ? `v${usage.system.panel_version}` : '—'}
                  sub={version?.update_available ? `update available: v${version.latest}` : 'up to date'} />
          </div>

          <h2 className="text-xs font-semibold uppercase tracking-wider text-slate-500 dark:text-slate-400 mb-2">Services</h2>
          {services.length === 0 ? (
            <div className="py-8 text-center text-sm text-slate-400">No service information.</div>
          ) : (
