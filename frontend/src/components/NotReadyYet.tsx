...
export default function NotReadyYet({
  ...
  const { t } = useTranslation('NotReadyYet')
  return (
    <div className="px-8 py-6">
      ...
      <div className="bg-white dark:bg-slate-800 border-2 border-dashed border-slate-200 dark:border-slate-700 rounded-2xl p-12 text-center">
        ...
        <h3 className="text-base font-semibold text-slate-900 dark:text-slate-100 mb-1">{t('construction.heading')}</h3>
        <p className="text-sm text-slate-500 dark:text-slate-500 max-w-md mx-auto">
          {t('construction.availablePre')}<span className="font-mono text-brand-700 dark:text-brand-300">{phase}</span>{t('construction.availablePost')}
        </p>
      </div>
    </div>
  )
