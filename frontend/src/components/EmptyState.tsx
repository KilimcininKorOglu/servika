export default function EmptyState({
  ...
  title: string
  description?: string
  button?: { label: string; onClick?: () => void; disabled?: boolean; hint?: string }
}) {
  return (
    <div className="py-16 flex flex-col items-center text-center px-4">
      ...
      {description && <p className="text-sm text-slate-500 dark:text-slate-500 max-w-md mb-5">{description}</p>}
      {button && (
        <button
